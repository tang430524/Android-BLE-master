...
public final class Ble<T extends BleDevice> {
     ...
     * @param callback 通知回调
     */
    public void enableNotifyByUuid(T device, boolean enable, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, BleNotifyCallback<T> callback){
        request.enableNotifyByUuid(device, enable, serviceUUID,serviceInstanceId, characteristicUUID, callback);
    }
     ...
     * @param callback 写入回调
     */
    public boolean readByUuid(T device, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID , BleReadCallback<T> callback){
        return request.readByUuid(device, serviceUUID,serviceInstanceId, characteristicUUID, callback);
    }

    public boolean readDesByUuid(T device, UUID serviceUUID, int serviceInstanceId,UUID characteristicUUID, UUID descriptorUUID , BleReadDescCallback<T> callback){
        DescriptorRequest<T> request = Rproxy.getRequest(DescriptorRequest.class);
        System.out.println("descriptorUUID:"+descriptorUUID);
        return request.readDes(device, serviceUUID,serviceInstanceId, characteristicUUID, descriptorUUID, callback);
    }

    public boolean writeDesByUuid(T device, byte[] data, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, UUID descriptorUUID, BleWriteDescCallback<T> callback){
        DescriptorRequest<T> request = Rproxy.getRequest(DescriptorRequest.class);
        return request.writeDes(device, data, serviceUUID,serviceInstanceId, characteristicUUID, descriptorUUID, callback);
    }
     ...
     * @param callback 写入回调
     */
    public boolean writeByUuid(T device, byte[]data, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, BleWriteCallback<T> callback){
        return request.writeByUuid(device, data, serviceUUID,serviceInstanceId, characteristicUUID, callback);
    }
