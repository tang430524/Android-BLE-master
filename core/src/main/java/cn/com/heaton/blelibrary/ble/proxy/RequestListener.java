...
public interface RequestListener<T> {
    ...
    void enableNotifyByUuid(T device, boolean enable, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, BleNotifyCallback<T> callback);
    ...
    boolean readByUuid(T device, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, BleReadCallback<T> callback);

    boolean readRssi(T device, BleReadRssiCallback<T> callback);

    boolean write(T device, byte[]data, BleWriteCallback<T> callback);

    boolean writeByUuid(T device, byte[]data, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, BleWriteCallback<T> callback);
