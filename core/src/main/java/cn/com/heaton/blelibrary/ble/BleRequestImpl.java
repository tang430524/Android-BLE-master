...
public final class BleRequestImpl<T extends BleDevice> {
    ...
    public boolean writeCharacteristicByUuid(String address, byte[] value, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID ) {
        BluetoothGatt bluetoothGatt = getBluetoothGatt(address);
        BluetoothGattCharacteristic characteristic = gattCharacteristic(bluetoothGatt, serviceUUID,serviceInstanceId, characteristicUUID);
    ...
    /**
     * 这是一个废弃的函数，通过uuid获取servi，然后再获取characteristic。
     * 在有相同serveiceUUID的时候存者不能使用重复service的问题。
     * by @Eleien
     * @param gatt
     * @param serviceUUID
    ...
    /**
     * 通过uuid和instanceId获取service，然后再获取characteristic。
     * 新增使用sevice的instanceId(即handle值)
     * by @Eleien
     * @param gatt
     * @param serviceUUID
     ...
     * @return
     */
    public BluetoothGattCharacteristic gattCharacteristic(BluetoothGatt gatt, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID ){
        if (gatt == null){
            BleLog.e(TAG, "BluetoothGatt is null");
            return null;
        }
        ...
        for (BluetoothGattService gattService:gattServiceList){/*避免有重复serviceUUID而造成其他service无法使用问题*/
            if (Integer.valueOf(gattService.getInstanceId()).equals(serviceInstanceId)
                    &&gattService.getUuid().equals(serviceUUID)) {
                BluetoothGattCharacteristic characteristic = gattService.getCharacteristic(characteristicUUID);
    ...
    public boolean readCharacteristicByInstanceId(String address, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID ) {
        BluetoothGatt bluetoothGatt = getBluetoothGatt(address);
        BluetoothGattCharacteristic gattCharacteristic = gattCharacteristic(bluetoothGatt, serviceUUID,serviceInstanceId, characteristicUUID);
        if (gattCharacteristic != null) {
    ...
    public boolean readDescriptorByInstanceId(String address, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, UUID descriptorUUID ){
        BluetoothGatt bluetoothGatt = getBluetoothGatt(address);
        BluetoothGattCharacteristic gattCharacteristic = gattCharacteristic(bluetoothGatt, serviceUUID,serviceInstanceId, characteristicUUID);
        if (gattCharacteristic != null){
    ...
    public boolean writeDescriptorByInstanceId(String address, byte[] data, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, UUID descriptorUUID ){
        BluetoothGatt bluetoothGatt = getBluetoothGatt(address);
        BluetoothGattCharacteristic gattCharacteristic = gattCharacteristic(bluetoothGatt, serviceUUID,serviceInstanceId, characteristicUUID);
        if (gattCharacteristic != null){
            BluetoothGattDescriptor descriptor = gattCharacteristic.getDescriptor(descriptorUUID);
    ...
    public void setCharacteristicNotificationByUuid(String address, boolean enabled, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID) {
        BluetoothGatt bluetoothGatt = getBluetoothGatt(address);
        BluetoothGattCharacteristic characteristic = gattCharacteristic(bluetoothGatt, serviceUUID,serviceInstanceId, characteristicUUID);
