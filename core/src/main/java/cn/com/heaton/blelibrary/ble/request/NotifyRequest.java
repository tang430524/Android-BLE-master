...
public class NotifyRequest<T extends BleDevice> implements NotifyWrapperCallback<T> {
    ...
    public void notifyByUuid(T device, boolean enable, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, BleNotifyCallback<T> callback) {
        notifyCallback = callback;
        bleRequest.setCharacteristicNotificationByUuid(device.getBleAddress(),enable, serviceUUID,serviceInstanceId, characteristicUUID);
    }
