...
public class DescriptorRequest<T extends BleDevice> implements DescWrapperCallback<T> {
    ...
    public boolean readDes(T device, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, UUID descriptorUUID, BleReadDescCallback<T> callback){
        this.bleReadDescCallback = callback;
        return bleRequest.readDescriptorByInstanceId(device.getBleAddress(), serviceUUID,serviceInstanceId, characteristicUUID, descriptorUUID);
    }

    public boolean writeDes(T device, byte[] data, UUID serviceUUID,int serviceInstanceId, UUID characteristicUUID, UUID descriptorUUID, BleWriteDescCallback<T> callback){
        this.bleWriteDescCallback = callback;
        return bleRequest.writeDescriptorByInstanceId(device.getBleAddress(), data, serviceUUID,serviceInstanceId, characteristicUUID, descriptorUUID);
    }
